...
#ifndef BOOST_BEAST_WEBSOCKET_GNUTLS_SSL_HPP
#define BOOST_BEAST_WEBSOCKET_GNUTLS_SSL_HPP
...
/** Tear down a `net::gnutls::stream`.
    ...
    `net::gnutls::stream`, callers are responsible for
...
teardown(
    role_type role,
    net::gnutls::stream<SyncStream>& stream,
    error_code& ec);

/** Start tearing down a `net::gnutls::stream`.
    ...
    `net::ip::tcp::socket` or any `net::gnutls::stream`,
...
async_teardown(
    role_type role,
    net::gnutls::stream<AsyncStream>& stream,
    TeardownHandler&& handler);

} // beast
} // boost

#include <boost/beast/websocket/gnutls/impl/ssl.hpp>
